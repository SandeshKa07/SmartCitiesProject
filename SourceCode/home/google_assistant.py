...
class MyAssistant(object):
    ...
    def __init__(self, acredentials, adevice_config, adevice_model_id, aproject_id, aquery, anickname, asocket_server_obj, aclient_address):
        ...
        self.socket_server_obj = asocket_server_obj
        self.client_address = aclient_address
    ...
    def _process_event(self, event):
        ...
        if event.type == EventType.ON_DEVICE_ACTION:
            ...
            try:
                print("I can send data to client_address = " + str(self.client_address))
                data = json.dumps({"light":lstatus, "heater":hstatus})
                #sent = self.socket_server_obj.sendto(assistant_control_status.encode('utf-8'), self.client_address)
                sent = self.socket_server_obj.sendto(data.encode(), self.client_address)
            except Exception as e:
                print("Exception occurred while sending : ")
                print(e)

        if event.type == EventType.ON_ASSISTANT_ERROR and event.args and event.args['is_fatal']:
            self.socket_server_obj.close()
            sys.exit(1)
...
def main():
    ...
    socket_server_obj = socket.socket(socket.AF_INET, socket.SOCK_DGRAM, socket.IPPROTO_UDP)
    ...
    smartcities_assistant = MyAssistant(args.credentials, args.device_config, "smartcitiesandiot", args.project_id, args.query,
                                        args.nickname, socket_server_obj, client_address)
